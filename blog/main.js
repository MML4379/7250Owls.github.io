...
async function fetchData() {
  try {
    ...
    if (!page) {
      ({ data, error } = await _supabase.from("bloginfo").select("*"));
      indexPage = true;
    } else {
    ...
    if (data.length === 0) {
      container.innerHTML =
        "<p>No blog posts found. Please reload, or try later.</p>";
    } else {
      const html = data
        .toReversed()
        .map((row) => {
...
function renderContent(post, indexPage) {
  ...
  if (indexPage) {
    const textOnly = content.replace(/<img[^>]*>/g, "").replace(/<[^>]+>/g, "");
  ...
  const maxImageBottom = Math.max(
    0,
    ...(post.Images || []).map((img) => img.bottomPx || 0),
  );
  const maxVideoBottom = Math.max(
    0,
    ...(post.Videos || []).map((v) => v.bottomPx || 0),
  );
